...
export const Navbar: React.FC = () => {
  ...
  return (
    <motion.header
      ...
      className="fixed top-3 left-0 right-0 mx-auto px-4 w-full max-w-6xl z-50 pointer-events-auto"
    >
      <div className="glass-panel-heavy rounded-2xl px-3 md:px-4 py-2 flex items-center justify-between border border-slate-700/60 shadow-2xl backdrop-blur-xl gap-2 w-full">
        ...
        <Link to="/" className="flex items-center gap-2 text-slate-100 hover:text-white transition-colors shrink-0" aria-label="Acoustic Shield Home">
          ...
          <span className="font-display text-base tracking-tight font-light text-slate-100 hidden sm:inline shrink-0">
            Acoustic Shield
          </span>
        </Link>

        {/* Center: Compact Navigation Tabs */}
        <nav className="flex items-center gap-0.5 bg-slate-900/80 p-1 rounded-xl border border-slate-800/80 min-w-0 shrink" aria-label="Main Navigation">
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path} aria-label={`Navigate to ${link.label}`}>
                <motion.div
                  ...
                  whileTap={{ scale: 0.97 }}
                  className={`px-2 md:px-2.5 py-1.5 rounded-lg text-xs font-mono flex items-center gap-1.5 transition-all ${
                    isActive
                ...
                >
                  ...
                  <span className={isActive ? 'inline text-xs' : 'hidden xl:inline text-xs'}>{link.label}</span>
                </motion.div>
              </Link>
            );
          })}
        ...
        {/* Right: 3D Scope Background Selector Button */}
        <div className="relative shrink-0" ref={dropdownRef}>
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
            onClick={() => setIsOpen(!isOpen)}
            className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white border border-indigo-400/40 px-2.5 py-1.5 rounded-xl text-xs font-mono font-medium transition-all shadow-md shadow-indigo-600/30 cursor-pointer shrink-0"
            aria-label="3D Background Mode Menu"
          >
            <Layers className="w-3.5 h-3.5 text-indigo-200 shrink-0" />
            <span className="hidden sm:inline">3D: {activeModeObj.label}</span>
                ...
                {modes.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-left font-mono transition-all cursor-pointer ${
                        isSelected
